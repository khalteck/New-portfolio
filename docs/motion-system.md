...
- CSS owns simple hover, focus, underline, and preloader keyframes.
- `requestAnimationFrame` owns cursor interpolation, particles, progress, and project-preview
  pointer tracking.
...
last 150–350 ms, route/menu transitions 600–800 ms, and scroll reveals 700–1100 ms. Transforms and
...
| ---------------------------------- | ------------------------------------------ | ----------------------------------------------- | -------------------------------------------------------------------------------------------------------- | ----------------------------------------------------------------------------- | ------------------------------------------------------------------ |
| Preloader letter sequence and exit | First page load in a browser tab session   | CSS stagger within a hard 1.45 s bound          | Letter `opacity/transform`, rule scale, overlay exit; timeout cleared on unmount and session flag stored | Same visual where motion is allowed; never receives focus or blocks semantics | Not rendered; content is available immediately                     |
...
| Hero line masks                    | Home hero mount                            | 700–1000 ms, `power3.out`, short stagger        | Line wrappers reveal through `translateY/opacity`; component-scoped timeline reverts                     | Smaller travel and safe fluid type prevent clipping                           | Final lines render immediately                                     |
| Hero supporting-content stagger    | After hero heading begins                  | 600–900 ms, `power2.out`                        | Intro, metrics, availability, and CTAs use transform/opacity; scoped cleanup                             | Stagger is tightened on compact layouts                                       | Final content renders immediately                                  |
| Navigation drawer panel            | Menu button opens/closes native dialog     | 600–800 ms, restrained cubic curve              | Overlay opacity and panel transform; dialog close removes scroll lock and restores trigger focus         | Right panel becomes a full-height narrow-screen surface                       | Dialog changes state without long reveal                           |
| Navigation menu stagger            | Drawer enters                              | 450–700 ms total                                | Per-item transform/opacity using stable item indices; closing dialog clears state                        | Shorter stagger on smaller screens                                            | Items appear immediately                                           |
| Philosophy reveal                  | Section enters near 88% viewport threshold | About 900 ms, `power3.out`                      | Scoped `translateY/opacity`; one-shot ScrollTrigger is reverted with its context                         | Same hierarchy with reduced travel                                            | Statement is visible without masking                               |
| About copy reveal                  | About blocks enter viewport                | 700–900 ms, `power3.out`                        | Block transform/opacity; scoped cleanup                                                                  | Stacked layout on small screens; no pointer dependency                        | Copy is immediately visible                                        |
| Technology item entrance           | Stack group enters viewport                | 700–900 ms with controlled stagger              | Item transform/opacity; scoped ScrollTrigger cleanup                                                     | Wrapped/staked groups use shorter travel                                      | Items render in final position                                     |
| Technology hover state             | Pointer hover or keyboard focus            | 150–250 ms CSS transition                       | Accent/rule/arrow transforms; no scripted lifecycle                                                      | Focus remains equivalent; touch receives stable presentation                  | State changes remain immediate or very short                       |
...
| Project row title/metadata         | Hover or focus of a published row          | 150–350 ms CSS/GSAP                             | Title fill/outline, metadata offset, rule scale; state cleared on leave/blur                             | Mobile/touch uses stable filled title and inline image                        | Focus/hover state remains without movement                         |
...
| Custom cursor follow               | Fine-pointer move at desktop width         | Continuous 0.17 interpolation                   | `translate3d` plus contextual data state; frame/listeners/class removed on cleanup                       | Not rendered under 1024 px or on coarse pointers                              | Not rendered; native cursor remains                                |
| Particle ambience                  | Allowed desktop mount                      | Continuous, deliberately slow                   | Canvas dots/lines; resize, visibility, and animation-frame listeners cleaned up                          | Only ≥1024 px with a fine pointer; capped density and device-pixel ratio      | Not rendered                                                       |
