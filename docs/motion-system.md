...
- Tailwind utilities own simple hover, focus, and underline states; root CSS owns shared preloader
  keyframes.
- `requestAnimationFrame` owns cursor interpolation, particles, progress, the homepage header
  illustration, and project-preview pointer tracking.
...
| Homepage header character journey | Homepage scroll position and direction   | One event-coalesced frame per scroll/resize update | KO morph plus articulated inline-SVG man and dog use deterministic launch, gait, climb, seated, and reverse poses; the seated details use compositor keyframes; observers, listeners, timers, and frames are removed on cleanup | Geometry is measured from KO, header border, Work, and the Résumé R on desktop and mobile | Deliberate exception: the complete scroll-linked sequence remains active |
...
- the homepage header illustration remains scroll-linked as a deliberate product exception;
...
- The header character journey uses the same phases on every supported width, derives figure height
  from the header geometry, and remains pointer-inert beneath the home and Résumé links.
